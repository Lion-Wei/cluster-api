...
// NewDeployer returns a deployer and sets kubeconfig path for the cluster to be deployed
func NewDeployer(kubeConfigPath string, machineSetupConfigPath string) *deployer {
	...
	ma, err := huawei.NewMachineActuator(token, nil, machineSetupConfigPath)
	if err != nil {
		glog.Exit(err)
	}
	return &deployer{
