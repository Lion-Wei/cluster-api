...
// Long term, we should retrieve the current status by asking k8s, huawei etc. for all the needed info.
...
func (hc *HuaweiClient) instanceStatus(machine *clusterv1.Machine) (instanceStatus, error) {
	currentMachine, err := util.GetMachineIfExists(hc.machineClient, machine.ObjectMeta.Name)
	if err != nil {
		return nil, err
	}
	...
	return hc.machineInstanceStatus(currentMachine)
}

// Sets the status of the instance identified by the given machine to the given machine
func (hc *HuaweiClient) updateInstanceStatus(machine *clusterv1.Machine) error {
	...
	m, err := hc.setMachineInstanceStatus(currentMachine, status)
	if err != nil {
		return err
	}

	_, err = hc.machineClient.Update(m)
	return err
}

// Gets the state of the instance stored on the given machine CRD
func (hc *HuaweiClient) machineInstanceStatus(machine *clusterv1.Machine) (instanceStatus, error) {
	...
	serializer := json.NewSerializer(json.DefaultMetaFactory, hc.scheme, hc.scheme, false)
...
func (hc *HuaweiClient) setMachineInstanceStatus(machine *clusterv1.Machine, status instanceStatus) (*clusterv1.Machine, error) {
